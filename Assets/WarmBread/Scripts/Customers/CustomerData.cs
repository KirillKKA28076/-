...
namespace WarmBread
{
    ...
    public sealed class CustomerData : ScriptableObject
    {
        ...
        public void Configure(string n, string hello, string tale, Color color, bool isChild = false)
        { displayName = n; greeting = hello; story = tale; coat = color; child = isChild; }
        public static CustomerData[] Defaults()
        {
            ...
            for (int i = 0; i < result.Length; i++)
            {
                ...
                result[i].Configure(names[i], hello[i], tales[i], Color.HSVToRGB(.06f + i * .095f, .25f, .37f + .035f * i), i == 2);
            }
            return result;
        }
    }
